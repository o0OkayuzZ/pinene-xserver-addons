...
vm.runInContext(code + '\nglobalThis.api={infectionAt,damageMultiplier,corruption,revives,tryRevive,initializePlayer,forceMaxHpState,createArmorVariantItem,setCorruption,tickChargeCompletion,getScore,setScore,SCORE,beginCharge,syncStrengthBoost};',sandbox);
...
test('post-hurt addon lethal damage still consumes revive and restores HP', () => {

  const p=entity(0);init(p,1);p.health.currentValue=0;afterEvents.entityHurt.emit({hurtEntity:p,damage:999,damageSource:{cause:'addon_magic'}});assert.equal(api.revives(p),0);assert.equal(api.corruption(p),1);assert.equal(p.health.currentValue,40);assert.equal(p.effects.resistance.amplifier,4);

});

test('death event fallback attempts emergency revive for addon kills', () => {

  const p=entity(0);init(p,1);p.health.currentValue=0;afterEvents.entityDie.emit({deadEntity:p,damageSource:{cause:'addon_void'}});assert.equal(api.revives(p),0);assert.equal(api.corruption(p),1);assert.equal(p.health.currentValue,40);
