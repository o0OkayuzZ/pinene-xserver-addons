...
for root in roots:
 ...
 for tex in ['astew','gstew','gmbucket']:atlas['texture_data']['pinene_golden_foods_'+tex]={'textures':'textures/pinene_golden_foods/'+tex}
...
stamp=datetime.datetime.now().strftime('%Y%m%d_%H%M%S_%f');backup=Path.home()/'Downloads/pinene-gf-backups'/('legacy_food_review_'+stamp);backup.mkdir(parents=True)
records=[]
...
(D/'LEGACY_FOOD_DEPLOYMENT.json').write_bytes(enc(report));print(json.dumps(report,ensure_ascii=True,indent=2))
