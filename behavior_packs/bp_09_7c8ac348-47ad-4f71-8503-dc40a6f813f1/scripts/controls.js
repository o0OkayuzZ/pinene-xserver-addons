  ...
  system.runInterval(() => {
    for (const player of world.getAllPlayers()) {
      try {
        ...
        if (state.charging && (!state.full || !state.holdingStemCell || state.combat)) {
          gear.cancel(player);
          sneakCharging.delete(player.id);
        ...
        if (state.charging) {
          ...
          show(player, `§a${"▰".repeat(filled)}§8${"▱".repeat(16-filled)} §f${(remaining/20).toFixed(1)}秒 §7| 蘇生 ${state.revives}/${state.maxRevives}`);
          continue;
        }
