...
const rewardSlots = vault().rewardDraw.selectedSlots;
assert.ok(rewardSlots.length >= 23 && rewardSlots.length <= 27);
assert.equal(mock.metrics.lootCalls, rewardSlots.length);
assert.equal(container.getItem(rewardSlots[0]).typeId, "minecraft:diamond");
