...
test("out-of-range Gaussian values are rejected and sampled again, without boundary clipping", () => {
    ...
    assert.equal(drawReconstructionDelayTicks("core", () => draws.shift()), 15000);
    assert.equal(draws.length, 0);
    assert.equal(drawReconstructionDelayTicks("scenery", () => 0), 4800);
});

test("broken RNG has a bounded fallback rather than blocking a server tick", () => {
    let calls = 0;
    assert.equal(drawReconstructionDelayTicks("scenery", () => { calls++; return 1; }), 4800);
