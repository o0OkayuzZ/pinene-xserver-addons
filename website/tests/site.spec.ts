...
test('category deep links, search, zero results and reset', async ({ page }, testInfo) => {
 ...
 await expect(page.locator('[data-content-card]:visible')).toHaveCount(catalogue.contents.filter(c => c.visibility === 'public' && c.category.some(category => category === 'collection')).length);
