...
test('Dungeons category navigation, expanded gear and recipe round trip', async ({ page }, info) => {
 ...
 await page.getByRole('navigation', { name: 'Dungeonsの種類別一覧' }).getByRole('link', { name: '防具 16' }).click();
 ...
 await expect(armor.locator('[data-guide-entry]:visible')).toHaveCount(16);
 await armor.locator('[data-guide-entry]').last().getByRole('link').click();
 await expect(page).toHaveURL(/database\/entries\/dungeons-phantom-leggings\/$/);
 await page.goBack();
