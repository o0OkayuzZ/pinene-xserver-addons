...
現在はアイテム・ブロック42件、レシピ5件、小さな機能3件を掲載しています。追加内容と検証は [FIELD_GUIDE_REPORT.md](FIELD_GUIDE_REPORT.md) を参照してください。
...
- `src/data/pack-registry.json`: 選定済み10パック。canonical keyはmanifestのheader UUIDです。全パック数ではありません。
...
`public/draft/private` はページ生成前の `src/lib/public-data.mjs` で分離し、さらにフィールドを明示的に選んでいます。検索はその公開済みカードだけを対象とします。draft候補は現状18件ありますが、固定件数を検証する仕様ではありません。非公開の実データをpublic/やクライアントスクリプトへ追加しないでください。
