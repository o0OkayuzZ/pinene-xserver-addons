...
const guideFile=new URL('../src/data/field-guide.json',import.meta.url);
const originalGuide=readFileSync(guideFile,'utf8');
...
try {
 ...
 const guide=JSON.parse(originalGuide);const guideFixture=guide.find(e=>e.visibility==='public');
 for(const visibility of ['private','draft'])guide.push({...structuredClone(guideFixture),id:visibility+'-guide-sentinel',name:'SECRET_SENTINEL',visibility});
 guideFixture.internalNote='SECRET_SENTINEL';
...
} finally { writeFileSync(file,original); writeFileSync(guideFile,originalGuide); run('node_modules/astro/bin/astro.mjs',['build']); }
run('scripts/check-output.mjs');
