...
> **2026-09-14 実機検証で適用不可。** BDS 1.26.45.1 は Beta APIs 有効でも独自 `sound_event` を LevelSoundEvent 列挙値として拒否する。以下の1.26.30条件は公式文書上の記載であり、対応確認済みバージョンではない。このブランチは未配備の修正案として保持する。実環境・mainのランタイムは移行前へ復元済み。詳細は [適用・復旧記録](../deployments/2026-09-14-pinecd-attempt.md)。

実機検証で判明した別件として、新しい形式のグループ名を `minecraft:itemGroup.name.record` に修正した（候補BP 1.0.29）。ただし音源名の受理エラーは残る。静的テストの成功は実行可能性を保証しない。
